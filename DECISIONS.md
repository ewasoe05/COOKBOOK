...
- **Week write on phones:** `/api/generate` and `/api/regenerate-meal` stream NDJSON with keep-alive pings (Safari buffers the first 1 KB) and set `maxDuration` to 300s. The client rewrites WebKit's raw `Load failed` / Chromium `Failed to fetch` into a stay-on-the-page retry. USDA lookups run in parallel with an 8s timeout so a hung FoodData Central call cannot take the whole week down.
