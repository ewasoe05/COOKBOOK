...
- **Claude model:** `claude-sonnet-4-6` as specified. JSON-only; one repair pass on Zod failure.
...
- **Everywhere / App Store:** ship as a hosted Next.js PWA first (Vercel + Add to Home Screen). An App Store binary should be a Capacitor/WKWebView shell of that HTTPS origin so the Anthropic key never ships in the client. Native wrappers stay deferred until there is a live domain and an Apple Developer account.
